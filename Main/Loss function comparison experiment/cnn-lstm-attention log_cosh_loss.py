...
def log_cosh_loss(y_true, y_pred):
    error = y_true - y_pred
    return tf.reduce_mean(tf.log(tf.cosh(error)))
...
def build_model_with_log_cosh_loss(lstm_units=512, cnn_filters=512, learning_rate=0.0001):
    ...
    model.compile(loss=log_cosh_loss, optimizer=optim, metrics=['mae'])
