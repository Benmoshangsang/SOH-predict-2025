...
def hyperparameter_optimization_pso(model, trainX, trainY):
    def objective(params):
        lstm_units, cnn_filters, learning_rate, batch_size = params
        model.set_params(lstm_units=int(lstm_units), cnn_filters=int(cnn_filters), learning_rate=learning_rate, batch_size=int(batch_size))
        model.fit(trainX, trainY)
        loss = model.score(trainX, trainY)  # Use negative loss to minimize it
        return -loss  # Particle Swarm Optimization minimizes the function

    lb = [256, 256, 1e-5, 16]  # Lower bounds for hyperparameters
    ub = [1024, 1024, 1e-2, 64]  # Upper bounds for hyperparameters

    best_params, _ = pso(objective, lb, ub, swarmsize=10, maxiter=10)
    print(f"Best Hyperparameters: {best_params}")
    return best_params

def build_model(lstm_units=512, cnn_filters=512, learning_rate=0.0001, alpha=0.5):
    ...
    dropout_lstm = Dropout(0.4)(attention_layer)
    ...
    concat = concatenate([batch_norm_lstm, batch_norm_cnn])
    flat = Flatten()(concat)
...
def main():
    ...
    testX_lstm, testY_lstm, v_testX_cnn, v_testY_cnn, i_testX_cnn, i_testY_cnn, t_testX_cnn, t_testY_cnn
        ...
        LSTM_layer = LSTM(512, activation='tanh', return_sequences=True)(input_LSTM)
        attention_layer = Attention()([LSTM_layer, LSTM_layer])
        dropout_lstm = Dropout(0.4)(attention_layer)
        batch_norm_lstm = BatchNormalization()(dropout_lstm)
        ...
        history = model.fit(
            x=[trainX_lstm, v_trainX_cnn, i_trainX_cnn, t_trainX_cnn],
            y=trainY_lstm,
            validation_data=([valX_lstm, v_valX_cnn, i_valX_cnn, t_valX_cnn], valY_lstm),
        ...
        val_loss = []
        val_results = model.evaluate([valX_lstm, v_valX_cnn, i_valX_cnn, t_valX_cnn], valY_lstm)
        val_loss.append(val_results)
        print('Val loss:', val_results)

        test_loss = []
        results = model.evaluate([testX_lstm, v_testX_cnn, i_testX_cnn, t_testX_cnn], testY_lstm)
        ...
        save_predictions_and_true_values(inv_testPredict_mpc, inv_testY, 'cnn_lstm_attention_predictions.csv')
