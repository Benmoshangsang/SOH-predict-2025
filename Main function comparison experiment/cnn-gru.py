...
import time
import psutil
from tensorflow.keras.layers import Dense, GRU, Conv1D, Flatten, Attention, Dropout, BatchNormalization
...
def adaptive_huber_loss(y_true, y_pred, delta=0.1):
    error = y_true - y_pred
    condition = tf.abs(error) <= delta
    small_res = 0.5 * tf.square(error)
    large_res = delta * (tf.abs(error) - 0.5 * delta)
    return tf.where(condition, small_res, large_res)

def multi_scale_wasserstein(y_true, y_pred):
    scales = [1, 2, 4]
    distance = 0.0
    for scale in scales:
        true_hist = tf.histogram_fixed_width(y_true, [tf.reduce_min(y_true), tf.reduce_max(y_true)], nbins=scale)
        pred_hist = tf.histogram_fixed_width(y_pred, [tf.reduce_min(y_pred), tf.reduce_max(y_pred)], nbins=scale)
        
        true_hist = tf.cast(true_hist, tf.float32) / tf.reduce_sum(tf.cast(true_hist, tf.float32))
        pred_hist = tf.cast(pred_hist, tf.float32) / tf.reduce_sum(tf.cast(pred_hist, tf.float32))

        distance += tf.reduce_sum(tf.abs(tf.cumsum(true_hist) - tf.cumsum(pred_hist)))
    return distance / len(scales)

def combined_loss(y_true, y_pred, delta=0.1, alpha=0.5):
    return alpha * adaptive_huber_loss(y_true, y_pred, delta) + (1 - alpha) * multi_scale_wasserstein(y_true, y_pred)

def mpc_control(true_values, predicted_values, lookahead=10, bounds=None):
    def cost_function(x, *args):
        true_values, current_predictions = args
        adjusted_predictions = current_predictions + x
        return np.sum((true_values - adjusted_predictions) ** 2)
    
    corrected_predictions = []
    for i in range(len(predicted_values) - lookahead):
        current_predictions = predicted_values[i:i + lookahead]
        current_true_values = true_values[i:i + lookahead]
        
        x0 = np.zeros(lookahead)
        
        result = minimize(cost_function, x0, args=(current_true_values, current_predictions), bounds=bounds)
        corrected_predictions.append(predicted_values[i] + result.x[0])
    
    return np.array(corrected_predictions)

def hyperparameter_optimization(model, trainX, trainY, param_grid):
...
def build_model(gru_units=512, cnn_filters=512, learning_rate=0.0001, alpha=0.5):
    input_CNN_v = Input(shape=(pr.seq_len_cnn, 10), name="CNN_Input_V")
    input_CNN_i = Input(shape=(pr.seq_len_cnn, 10), name="CNN_Input_i")
    input_CNN_t = Input(shape=(pr.seq_len_cnn, 10), name="CNN_Input_t")
    input_GRU = Input(shape=(pr.seq_len_lstm, 10), name="GRU_Input")

    GRU_layer = GRU(gru_units, activation='tanh', return_sequences=True)(input_GRU)
    attention_layer = Attention()([GRU_layer, GRU_layer])
    dropout_gru = Dropout(0.4)(attention_layer)
    batch_norm_gru = BatchNormalization()(dropout_gru)
    ...
    concat = concatenate([batch_norm_gru, batch_norm_cnn])
    flat = Flatten()(concat)
    ...
    model = Model(inputs=[input_GRU, input_CNN_v, input_CNN_i, input_CNN_t], outputs=output)

    optim = Adam(learning_rate=learning_rate)

    model.compile(loss=combined_loss, optimizer=optim, metrics=['mae'])
...
def main():
    ...
    for j, (train_idx, val_idx) in enumerate(folds):
        ...
        trainX_gru, trainY_gru, SS_tr_gru = utils.extract_VIT_capacity(train_x_data, train_y_data, pr.seq_len_lstm, pr.hop, pr.sample, c=True, slicing=slicing, filp=filp)
        valX_gru, valY_gru, SS_val_gru = utils.extract_VIT_capacity(val_x_data, val_y_data, pr.seq_len_lstm, pr.hop, pr.sample, c=True, slicing=slicing, filp=filp)
        testX_gru, testY_gru, SS_tt_gru = utils.extract_VIT_capacity(test_x_data, test_y_data, pr.seq_len_lstm, pr.hop, pr.sample, c=True, slicing=slicing, filp=filp)
        ...
        trainX_gru, trainY_gru, v_trainX_cnn, v_trainY_cnn, i_trainX_cnn, i_trainY_cnn, t_trainX_cnn, t_trainY_cnn = align_batches_by_min_length(
            trainX_gru, trainY_gru, v_trainX_cnn, v_trainY_cnn, i_trainX_cnn, i_trainY_cnn, t_trainX_cnn, t_trainY_cnn
        )


        valX_gru, valY_gru, v_valX_cnn, v_valY_cnn, i_valX_cnn, i_valY_cnn, t_valX_cnn, t_valY_cnn = align_batches_by_min_length(
            valX_gru, valY_gru, v_valX_cnn, v_valY_cnn, i_valX_cnn, i_valY_cnn, t_valX_cnn, t_valY_cnn
        )


        testX_gru, testY_gru, v_testX_cnn, v_testY_cnn, i_testX_cnn, i_testY_cnn, t_testX_cnn, t_testY_cnn = align_batches_by_min_length(
            testX_gru, testY_gru, v_testX_cnn, v_testY_cnn, i_testX_cnn, i_testY_cnn, t_testX_cnn, t_testY_cnn
        )
        ...
        input_GRU = Input(shape=(pr.seq_len_lstm, trainX_gru.shape[-1]), name="GRU_Input")

        GRU_layer = GRU(512, activation='tanh', return_sequences=True)(input_GRU)
        attention_layer = Attention()([GRU_layer, GRU_layer])
        dropout_gru = Dropout(0.4)(attention_layer)
        batch_norm_gru = BatchNormalization()(dropout_gru)

        CNN_layer_v = Conv1D(512, 5, activation='relu', strides=1, padding="same")(input_CNN_v)
        CNN_layer_i = Conv1D(512, 5, activation='relu', strides=1, padding="same")(input_CNN_i)
        CNN_layer_t = Conv1D(512, 5, activation='relu', strides=1, padding="same")(input_CNN_t)

        concat_cnn = concatenate([CNN_layer_v, CNN_layer_i, CNN_layer_t])
        CNN_fusion = Conv1D(512, 5, activation='relu', strides=1, padding='same')(concat_cnn)
        dropout_cnn = Dropout(0.4)(CNN_fusion)
        batch_norm_cnn = BatchNormalization()(dropout_cnn)
        ...
        history = model.fit(
            x=[trainX_gru, v_trainX_cnn, i_trainX_cnn, t_trainX_cnn],
            y=trainY_gru,
            validation_data=([valX_gru, v_valX_cnn, i_valX_cnn, t_valX_cnn], valY_gru),
            batch_size=16,  # Reduced batch size
            epochs=500,  # Increased epochs for thorough training
            callbacks=[tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=30, restore_best_weights=True)]  # Increased patience
        )
        ...
        val_loss = []
        val_results = model.evaluate([valX_gru, v_valX_cnn, i_valX_cnn, t_valX_cnn], valY_gru)
        val_loss.append(val_results)
        print('Val loss:', val_results)

        test_loss = []
        results = model.evaluate([testX_gru, v_testX_cnn, i_testX_cnn, t_testX_cnn], testY_gru)
        test_loss.append(results)
        print('Test loss:', results)

        valPredict = model.predict([valX_gru, v_valX_cnn, i_valX_cnn, t_valX_cnn])
        testPredict = model.predict([testX_gru, v_testX_cnn, i_testX_cnn, t_testX_cnn])

        inv_valY = SS_val_gru.inverse_transform(valY_gru)
        inv_valPredict = SS_val_gru.inverse_transform(valPredict)
        inv_testY = SS_tt_gru.inverse_transform(testY_gru)
        inv_testPredict = SS_tt_gru.inverse_transform(testPredict)
        ...
        save_predictions_and_true_values(inv_testPredict_mpc, inv_testY, 'cnn_gru_predictions.csv')
