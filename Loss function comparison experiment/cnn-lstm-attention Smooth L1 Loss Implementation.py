...
def smooth_l1_loss(y_true, y_pred, beta=1.0):
    error = y_true - y_pred
    condition = tf.abs(error) < beta
    small_res = 0.5 * tf.square(error)
    large_res = tf.abs(error) - 0.5 * beta
...
def build_model_with_smooth_l1_loss(lstm_units=512, cnn_filters=512, learning_rate=0.0001):
    ...
    model.compile(loss=smooth_l1_loss, optimizer=optim, metrics=['mae'])
