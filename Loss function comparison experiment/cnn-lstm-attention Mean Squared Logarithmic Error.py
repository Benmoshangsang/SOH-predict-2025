...
def msle_loss(y_true, y_pred):
    return tf.reduce_mean(tf.square(tf.math.log(y_pred + 1) - tf.math.log(y_true + 1)))
...
def build_model_with_msle_loss(lstm_units=512, cnn_filters=512, learning_rate=0.0001):
    ...
    model.compile(loss=msle_loss, optimizer=optim, metrics=['mae'])
